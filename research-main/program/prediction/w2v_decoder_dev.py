...
def model_w2v(seqs, out_path):
  ...
  for i in range(6):
    model = word2vec.Word2Vec(sentences=seqs, vector_size=64, window=i, min_count=0, epochs = 100)
    os.makedirs(out_path, exist_ok = True)
    model.wv.save_word2vec_format(out_path + "/w2v_model_{}-mer.pt".format(i), binary=True)
