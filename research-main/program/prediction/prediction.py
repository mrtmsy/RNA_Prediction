...
class DeepProcess():
    ...
    def model_prediction(self, data_sets):
        ...
        for i, (token, label) in enumerate(loader):
            pe = get_pe(torch.full((token.size(0), 1), self.params.seq_len).to(
                self.device), self.params.seq_len, self.device).float().to(self.device)
...
model_path = base_path + "/model/data_model/deep_model.pt"
